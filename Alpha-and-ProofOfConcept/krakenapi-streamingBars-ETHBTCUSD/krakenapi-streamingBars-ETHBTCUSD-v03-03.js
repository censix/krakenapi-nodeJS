	...
	kraken.api('AddOrder', 
			...
			function(error, data) {
				if(error) {
					...
					//causes duplication when error.code='ESOCKETTIMEDOUT'// setTimeout(function(){ placeorderBRACKET(tradepairname, qty, price, price2); },5*1000);
				} else { 
					console.log(data.result.descr);
					console.log(data.result.txid);
				}
	...
	kraken.api('AddOrder', 
			...
			function(error, data) {
				if(error) {
					...
					//causes duplication when error.code='ESOCKETTIMEDOUT'// setTimeout(function(){ placeorderEXIT(tradepairname, qty); },5*1000);
				} else { 
					console.log(data.result.descr);
					console.log(data.result.txid);
				}
...
console.log('pair2 last time+Op+Vo '+ohlc2.time.slice(-1)[0] + ' '+ ohlc2.Op.slice(-1)[0] + ' '+ ohlc2.Vo.slice(-1)[0]);
	...
	if (glob.TTLbars!=undefined) { if (glob.TTLbars>0) glob.TTLbars = glob.TTLbars-1; };
	...
	if (RresultObj.SIGNAL == 0) { 
		...
		//
		//glob_prev = glob; // Save existing trade parameters
...
//return 0; //TEST
	...
	if (cur_qty!=0 && glob.entryAt!=undefined) { 
		if ( cur_qty>0 && Number(Op_price1)/glob.entryAt > (1.025 + 0.01) ) glob.TTLbars = 0; 
	};	
	
	// Take order placing decisions
	if (cur_qty==0) {
		if (currentSIGNAL && glob.target_direction != 0 ) {
		...
		glob.TTLbars  = 0;
		console.log('glob.status '+glob.status);		
	} else if (glob.TTLbars > 0) {  // ADJUST TARGET AND STOPLOSS, reset TTL, if target is greater AND phat is greater
		if ( currentSIGNAL && (glob.target_price > glob_prev.target_price) && (glob.phat>glob_prev.phat) ) { //ADJUST upwards and extend duration
		...
		if (Number(Op_price1) > glob.entryAt*1.005) { // MOVE STOP into the profit zone as soon as possible!!
			glob.stoploss_price = glob.entryAt*1.005;
			console.log('Moved stoploss to zero-loss zone.')
		}
...
tradeqty = 1.0; //quanity to trade = ETH
tradeleverage = 2; //leverage to use. must be integer >1, otherwithse OpenOrders does not work.
// Initialize
ohlc = [];
prev_ohlc = [];
prev_ts = [];
...
console.log('Waiting for start of next 5 minute interval = freq at which bars are queried < barsize');
while(Math.floor(Date.now()/1000)%(5*60)!=0){};
...
//
	...
	},5*60*1000); // Requesting bar data every 15 seconds. May need to increase frequency here. 5 sec gives pacing error
