...
console.log('pair2 last time+Op+Vo '+ohlc2.time.slice(-1)[0] + ' '+ ohlc2.Op.slice(-1)[0] + ' '+ ohlc2.Vo.slice(-1)[0]);
	...
	RresultObj = null;
	RresultObj = R('./playingwith-RF/krakensim-RF-A-v035b-SIGNAL.R').data( ohlc0, ohlc1, ohlc2 ).callSync();
	...
	if (RresultObj.SIGNAL == 0) { 
		...
		glob.target_price = price * (1 + 0.018 - 0.0051);  // = trained target(%) - 1/2 * avg. spread (%)
...
//return 0; //TEST
	...
	if (cur_qty==0) {
		...
		glob.TTLbars  = 0;
		console.log('glob.status '+glob.status);		
	} else if (glob.TTLbars > 0 ) {  // ADJUST TARGET AND STOPLOSS, reset TTL, if target is greater
		if ( false && glob.target_price > glob_prev.target_price) { //ADJUST upwards - OFF
...
tradeqty = 0.40; //quanity to trade = ETH
